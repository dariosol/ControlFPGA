...
int main(int argc, char *argv[]) {
  ...
  //Elimino gli indirizzi uguali consecutivi; 
  //CHOD
  ...
  //Try to delete all equal addresses (also not sorted);
  cout<<"Elimino eventi con indirizzo uguale non consecutivi"<<endl;
 for(int i=0; i< CHODprim.timestamp.size();i++){
   CHODaddress = L0address(CHODprim.timestamp.at(i),CHODprim.finetime.at(i),bitfinetime);
   //   cout<<"****CHOD address "<<CHODaddress<<" ";
   //cout<<"Timestamp: "<<hex<<CHODprim.timestamp.at(i)<<" finetime "<<hex<<CHODprim.finetime.at(i)<<endl<<endl;
   for(int j=0; j< CHODprim.timestamp.size();j++){
     if (i==j) continue;
     CHODaddress1 = L0address(CHODprim.timestamp.at(j),CHODprim.finetime.at(j),bitfinetime);

     if (CHODaddress1 ==CHODaddress && j > i && TMath::Abs(CHODprim.time.at(i) - CHODprim.time.at(j)) < 1000){
       //      cout<<"i "<<i <<" j "<<j<<"CHOD ADDRESS : "<<hex<<CHODaddress<<" Next address "<<hex<<CHODaddress1<<" delete "<<hex<<CHODaddress<<endl;
       //	 cout<<"Timestamp: "<<hex<<CHODprim.timestamp.at(i)<<" finetime "<<hex<<CHODprim.finetime.at(i)<<endl;
       CHODprim.timestamp.erase(CHODprim.timestamp.begin()+(i));
       CHODprim.finetime.erase(CHODprim.finetime.begin()+(i));
       CHODprim.primitiveID.erase(CHODprim.primitiveID.begin()+(i));
       CHODprim.time.erase(CHODprim.time.begin()+(i));
       i=i-1;
       break;
     }
   }
  ...
  for(int i=0; i< CHODprim.timestamp.size();i++){
	...
	previousCHODaddress = CHODaddress;
	if(TMath::Abs(CHODprim.time.at(i)-MUVprim.time.at(j))<timecut){
